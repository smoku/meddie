...
The app is provider-agnostic — it builds from the Dockerfile at the repo root and runs anywhere a Linux container can run, given the dependencies below.

### Container

- Build via the repo's `Dockerfile` (multi-stage: Elixir release → minimal runtime image with `poppler-utils` for PDF parsing).
- Listens on `PORT` (default `8080`).
- Entrypoint runs the release migrate task before starting the server.

### PostgreSQL

- **Version**: 17 or newer.
- **Required extensions**: `citext` (case-insensitive emails), `vector` (pgvector for memory embeddings, HNSW index support).
- Note: the standard `postgres:N` Docker images do **not** bundle pgvector. Use `pgvector/pgvector:pgN-…` or install the extension manually.
- Connection via `DATABASE_URL` env var.
- Migrations run automatically on app startup.

### Object Storage

- Any **S3-compatible** service that supports presigned GET URLs.
- Used via `ex_aws_s3`. Endpoint is configurable through env vars (see below).
- Storage class for new uploads is configurable via `STORAGE_CLASS`; if unset, the bucket's default class is used.
- Buckets must be private — clients access objects through short-lived presigned URLs (default 15 min) generated by the Phoenix backend.

### Reverse proxy / TLS

- The app trusts `X-Forwarded-Proto`, so any HTTPS-terminating reverse proxy in front of it works.
- `force_ssl` redirects HTTP to HTTPS in production.
...
| `PORT` | HTTP port the app binds to (default `8080`) |
...
| `S3_REGION` | Object storage region (e.g. `auto` or a provider-specific region code) |
| `BUCKET_NAME` | Bucket name (default `meddie-documents`) |
| `STORAGE_CLASS` | Optional S3 storage class for new uploads |
